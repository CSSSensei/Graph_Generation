#ifndef QT_PROJECT_VERTEX_H
#define QT_PROJECT_VERTEX_H
#include <memory>
#include <vector>
#include <string>
#include "Edge.h"
...
public:
    ...
    Vertex(Vertex<T>&& vertex) noexcept  // Перемещающий конструктор
    ...
    Vertex<T>& operator=(Vertex<T>&& vertex) noexcept {  // Перемещающий оператор присваивания
        if (this != &vertex) {
            _name = std::move(vertex._name);
            _data = std::move(vertex._data);
            _input_edges = std::move(vertex._input_edges);
